...
check("Hero whitespace generous", "padding:6rem 0 3rem" in css)
check("Gradients quiet and center-safe (OG-like)", css.count("gradient")>=2 and css.count("gradient")<=10, f"gradients={css.count('gradient')}")
check("Card radius unified 14px", "--r:14px" in css, "r var")
check("Two accents for sweep (acc and acc2)", css.count("--acc:")==1 and css.count("--acc2:")==1)
check("Sections transparent, no stripe (OG-like)", ".section.alt{background:transparent" in css and "background:transparent" in css)
...
if fails:
    ...
    print("Human feel: calm whitespace (6rem hero), two accents with sweep, 14px radius, transparent sections, 1.7 leading, muted hierarchy. Gradients quiet and center-safe, no loud noise.")
